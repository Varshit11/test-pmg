...
from database_writer.aws_data_discovery import AWSRDSDiscovery, discover_aws_mysql_database
...
class MySQLConnectionManager:
    ...
    def __init__(self, connection_config: Dict[str, Any]):
        ...
        # Set default connection parameters for production (PyMySQL compatible)
        default_params = {
            'charset': 'utf8mb4',
            'connect_timeout': 30,
            'autocommit': True,
        ...
        # Filter out problematic parameters and merge
        filtered_params = {k: v for k, v in self.connection_params.items() 
                         if k not in ['sslmode', 'sql_mode']}
        self.connection_params = {**default_params, **filtered_params}
...
class DatabaseWriter:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        # Initialize schema manager
        self.table_name = self.db_config.get('table_name', 'T_PROJECT_ISSUES_SIMILAR')
        self.schema_manager = MySQLSchemaManager(self.db_config, self.table_name)
    ...
    def _get_database_config(self) -> Dict[str, Any]:
        """
        Get database configuration from settings (skip AWS discovery)
    ...
    def _get_insert_sql(self) -> str:
        """
        Get INSERT SQL statement with ON DUPLICATE KEY UPDATE for upsert
        
        Returns:
            SQL INSERT statement with upsert capability
        """
        # Define field order (excluding auto-generated fields)
        fields = [
            'project_id', 'issue_id', 'short_description', 'long_description', 'project_issue_class',
        ...
        # Always use upsert logic - replace existing records with same project_id + issue_id
        # Create ON DUPLICATE KEY UPDATE clause
        update_clauses = []
        for field in fields:
            if field not in ['project_id', 'issue_id']:  # Don't update key fields
    ...
    def _insert_batch(self, records: List[Dict[str, Any]]) -> int:
        """
        Insert/Update a batch of records into the database using upsert
        ...
        try:
            ...
            self.logger.info(f"Successfully upserted {len(data_tuples)} records (affected rows: {affected_rows})")
            return len(data_tuples)  # Return number of records processed
            
        except Exception as e:
            self.logger.error(f"Error upserting batch: {e}")
            raise DatabaseWriterError(f"Failed to upsert batch: {e}")
    ...
    def run(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Step 2: Skip database creation since it already exists
            self.logger.info("Database already exists, skipping creation...")
            ...
            # Step 6: Write data to database using upsert
            stats = self._write_data_to_db(records)
